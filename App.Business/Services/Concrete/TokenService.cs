namespace App.Business.Services.Concrete
{
    public class TokenService : ITokenService
    {
        ...
        public async Task<IDataResult<TokenDto>> CreateAccessTokenAsync(IdentityUser identityUser, AuditablePersonBaseEntityDto auditablePersonBaseEntityDto)
        {
            ...
            if (issuerSigningSymmetricSecurityKey is null) return new ErrorDataResult<TokenDto>(stringLocalizer[Messages.Token_Could_Not_Generated]);

            var signingCredentials = new SigningCredentials(issuerSigningSymmetricSecurityKey, SecurityAlgorithms.HmacSha256);
            if (signingCredentials is null) return new ErrorDataResult<TokenDto>(stringLocalizer[Messages.Token_Could_Not_Generated]);

            var claims = await GenerateIdentityUserClaimsAsync(identityUser, auditablePersonBaseEntityDto);
            if (claims is null) return new ErrorDataResult<TokenDto>(stringLocalizer[Messages.Token_Could_Not_Generated]);
            ...
            if (accessToken is null) return new ErrorDataResult<TokenDto>(stringLocalizer[Messages.Token_Could_Not_Generated]);

            return new SuccessDataResult<TokenDto>(new TokenDto { AccessToken = accessToken, Expiration = jwtSecurityToken.ValidTo }, stringLocalizer[Messages.Token_Was_Generated_Successfully]);
        }
    }
}